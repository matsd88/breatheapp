...
struct WatchHomeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 12) {
                // Heart Rate Suggestion Banner (shows when HR is elevated)
                if heartRateMonitor.shouldSuggestBreathing {
                    HeartRateSuggestionBanner(
                        heartRate: heartRateMonitor.currentHeartRate,
                        onDismiss: { heartRateMonitor.dismissSuggestion() }
                    )
                }
...
// MARK: - Heart Rate Suggestion Banner

struct HeartRateSuggestionBanner: View {
    let heartRate: Int
    let onDismiss: () -> Void
        ...
        NavigationLink(destination: WatchBreathingView()) {
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text("HR elevated: \(heartRate) bpm")
                        .font(.caption2.bold())
...
struct MotivationCard: View {
    ...
    private static let messages = [
        ...
        "1 minute can change your day",
        "You deserve some peace",
        "Start small, feel great",
        "Your mind will thank you"
    ]

    init() {
        _message = State(initialValue: Self.messages.randomElement() ?? "Take a breath")
    }
